...
$(document).ready(function () {
    ...
    $("form").submit(function (e) {
		...
		getRelatedShipmentsAddresses(parentPackages).then(relatedShipments => {
			ShowMessage("Your contract is deploying...",
				"It might take some time, please wait while it is done. The window will be automaticaly closed.");
...
function ChangeMessageText(message, submessage) {
    $("#modal-window .modal-text").text(message);
...
function ShowError(message, submessage) {
    ShowMessage(message, submessage, true)
    $("#modal-window .modal-text").addClass("error");   
}

function HideMessage() {
    $("#modal-window .close-modal-hidden").click();
}
...
function deployDeliveryContract(deviceKey, info, parentDeliveries, callback) {
    ...
    let promise = typeof (ethereum) !== "undefined" ? ethereum.enable() : Promise.resolve();
	promise.then(() => {
		...
		web3.eth.estimateGas({ data: bytecode }, function (error, estimatedGas) {
			...
			let createdDeliveryContract = deliveryContract.new(info, deviceKey, parentDeliveries, {
				...
				if (!err) {
					...
					HideMessage();
					setTimeout(() => ShowError("The contract deployment failed.", `Origin message: ${err}`, true), 200);
